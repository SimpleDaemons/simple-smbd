#!/bin/bash
# Create DMG installer for simple-smbd
...
VERSION="${1:-1.0.0}"
DMG_NAME="$simple-smbd-${VERSION}"
DMG_TEMP="dmg-temp"
DMG_FILE="${DMG_NAME}.dmg"
APP_NAME="$simple-smbd.app"
PKG_NAME="$simple-smbd.pkg"
...
if [ -f "../assets/icons/$simple-smbd.icns" ]; then
    # Set custom icon (requires additional tools)
    echo "Custom icon available at ../assets/icons/$simple-smbd.icns"
fi
