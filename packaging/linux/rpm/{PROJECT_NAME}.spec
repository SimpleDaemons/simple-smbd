...
Version:        ${VERSION}
Release:        1%{?dist}
Summary:        simple-smbd - A lightweight and secure daemon
License:        Apache-2.0
...
%description
simple-smbd is a modern, secure implementation of the SMB protocol.
...
if [ "$1" -eq 1 ]; then
    ...
    # Create service user
    if ! id smbddev &>/dev/null; then
        useradd -r -s /sbin/nologin -d /var/lib/%{name} -c "%{name} service user" smbddev
    fi
fi
