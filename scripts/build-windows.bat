...
if "%ENABLE_STATIC_LINKING%"=="ON" (
    ...
    set "STATIC_DIR=%DIST_DIR%\simple-smbd--static-windows"
    ...
    cd /d "%DIST_DIR%"
    powershell -Command "Compress-Archive -Path 'simple-smbd--static-windows' -DestinationPath 'simple-smbd--static-windows.zip' -Force"
    rmdir /s /q "simple-smbd--static-windows"
    
    call :print_success "Static binary package created: simple-smbd--static-windows.zip"
)
goto :eof
