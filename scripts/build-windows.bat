...
:build_project
call :print_status "Building simple-smbd..."
...
:install_project
if "%INSTALL%"=="true" (
    call :print_status "Installing simple-smbd..."
    cd /d "%BUILD_DIR%"
...
if "%ENABLE_STATIC_LINKING%"=="ON" (
    ...
    set "STATIC_DIR=%DIST_DIR%\simple-smbd-0.1.0-static-windows"
    if not exist "%STATIC_DIR%" mkdir "%STATIC_DIR%"
    
    REM Copy binary and files
    copy "%BUILD_DIR%\simple-smbd.exe" "%STATIC_DIR%\"
    ...
    cd /d "%DIST_DIR%"
    powershell -Command "Compress-Archive -Path 'simple-smbd-0.1.0-static-windows' -DestinationPath 'simple-smbd-0.1.0-static-windows.zip' -Force"
    rmdir /s /q "simple-smbd-0.1.0-static-windows"
    
    call :print_success "Static binary package created: simple-smbd-0.1.0-static-windows.zip"
)
goto :eof

REM Function to show usage
:show_usage
echo simple-smbd - Windows Build Script
echo.
...
REM Main execution
:main
call :print_status "Starting simple-smbd build process..."
...
call :print_success "simple-smbd build process completed successfully!"
goto :end
