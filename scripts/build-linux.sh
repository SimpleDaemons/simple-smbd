#!/bin/bash
# Generic Linux build script for simple-smbd
...
create_service() {
    if [ "$SERVICE" = "true" ]; then
        ...
        sudo tee /etc/systemd/system/simple-smbd.service > /dev/null << EOF
[Unit]
Description=Simple SMB CIFS Daemon
...
User=simple-smbd
Group=simple-smbd
ExecStart=/usr/local/bin/simple-smbd --daemon
Restart=always
RestartSec=5
...
EOF
        ...
        sudo useradd --system --no-create-home --shell /bin/false simple-smbd 2>/dev/null || true
        ...
        print_status "Use 'sudo systemctl start simple-smbd' to start the service"
    fi
}
...
show_usage() {
    cat << EOF
simple-smbd - Linux Build Script
