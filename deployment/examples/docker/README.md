# simple-smbd Docker Deployment

This directory contains Docker deployment examples for simple-smbd.

## Quick Start

1. **Build the Docker image:**
   ```bash
   docker build -t simple-smbd:latest .
   ```
...
| Volume | Description |
|--------|-------------|
| `./config:/etc/simple-smbd:ro` | Configuration files (read-only) |
| `./data:/var/lib/simple-smbd` | Data directory |
| `./logs:/var/log/simple-smbd` | Log files |
...
# View logs
docker-compose logs -f simple-smbd

# Restart service
docker-compose restart simple-smbd
...
The service uses a custom bridge network (`simple-smbd-network`) with subnet `172.22.0.0/16`.
...
# Check configuration
docker-compose exec simple-smbd cat /etc/simple-smbd/simple-smbd.conf
```
